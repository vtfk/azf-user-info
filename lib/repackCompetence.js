...
const repackWorkExperience = workExperience => {
  ...
  // Override
  expString = `Her er første variant: ${nl}, her er andre variant med dobbel escape: ${nl2}, her er tredje variant med trippel escape ${nl3}. Her er med frippel escape ${nl4}. Her er bare br ${br}, her er dobbel n ${doubleN}. Her er asci med stor A ${asciN}, her er asci med liten a ${asciSmallA}`
...
const repackEducation = education => {
  ...
  for (const edu of education) {
    eduString += `- ${edu.fromYear} - ${edu.toYear}. ${edu.degree} ${edu.subject} - ${edu.school}${nl}`
  }
  return eduString
}
...
const repackCertifications = certifications => {
  ...
  for (const cert of certifications) {
    certString += `- ${cert.year} - ${cert.yearEnd}. ${cert.type}: ${cert.name}${nl}`
  }
  return certString
}
...
const repackExperience = experience => {
  ...
  for (const exp of experience) {
    expString += `- ${exp.fromYear} - ${exp.toYear}. ${exp.position}: ${exp.organization}${nl}`
  }
  return expString
}
...
const removeSpecialCharacters = (str) => {
  return str.replace(/[^a-zA-Z0-9ÆØÅæøå .,-@&%()öäü$àèùñ\"\'\\\/]/g, "")
}
