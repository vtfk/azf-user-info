...
const getStudentPersons = async () => {
  const res = await fintRequest('utdanning/elev/person')
  logger('info', ['fintStudent', 'studentPerson', `repacking ${res._embedded._entries.length} persons`])
  const persons = res._embedded._entries.map(person => {
    const repacked = {
      ...
      elevSystemId: repackLinkResource(person._links?.elev, '1') ?? unknownValue
    }
    return repacked
  })
  return persons
...
module.exports = { getResourceCategoryCodes, getEmployeepositionCodes, getStudentPersons, getEmployeeResources, getEmployeePositions, getOrganizationElements }
