...
// https://graph.microsoft.com/v1.0/users?$select=userType,userPrincipalName,onPremisesSamAccountName,officeLocation,city,onPremisesSyncEnabled,mobilePhone,onPremisesExtensionAttributes&$filter=userType eq 'Member' and onPremisesSyncEnabled eq true and officeLocation ne null and endsWith(userPrincipalName,'@vtfk.no')&$count=true&$top=999
const getAzureAdPersons = async (options) => {
  const baseUrl = 'https://graph.microsoft.com/v1.0/users'
  const selectUrl = '$select=userType,userPrincipalName,onPremisesSamAccountName,displayName,officeLocation,city,onPremisesSyncEnabled,mobilePhone,onPremisesExtensionAttributes'
