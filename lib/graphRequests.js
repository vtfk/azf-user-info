...
const getAzureAdPersons = async (options) => {
  ...
  const selectUrl = '$select=userType,userPrincipalName,onPremisesSamAccountName,officeLocation,onPremisesSyncEnabled,mobilePhone,onPremisesExtensionAttributes'
  ...
  let url = `${baseUrl}?${selectUrl}&${filterUrl}&${countUrl}&${topUrl}&${expandUrl}`
