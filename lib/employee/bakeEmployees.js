...
const bakeEmployeeData = (resourceCategoryCodes, employeePositionCodes, employeePersons, employeeResources, employeePositions, organizationElements, azureAdPersons) => {
  ...
  const getOrgInfo = (unitId) => {
    ...
    unit.struktur = [ { organisasjonsId: unit.organisasjonsId, navn: unit.navn } ]

    // Get leader and organizaitonal structure
    let current = unit
    if (current) {
      const manager = employeePersons.find(person => person.ansattnummer !== unknownValue && person.ansattnummer === current.leder)
      if (manager) unit.leder = { ansattnummer: manager.ansattnummer, navn: `${manager.fornavn} ${manager.etternavn}` }
    }
    ...
    while (current && current.overordnet !== current.organisasjonsId) {
      ...
      unit.struktur.push( { organisasjonsId: current.organisasjonsId, navn: current.navn } )
    }
