...
const bakeEmployeeData = (resourceCategoryCodes, employeePositionCodes, employeePersons, employeeResources, employeePositions, organizationElements, azureAdPersons) => {
  ...
  const getOrgInfo = (unitId) => {
    ...
    if (current) {
      ...
      unit.struktur = [ { organisasjonsId: unit.organisasjonsId, navn: unit.navn, leder: managerAzure ? managerAzure.userPrincipalName : 'Ukjent leder', ledernavn: managerAzure ? managerAzure.displayName : 'Ukjent ledernavn' } ]
    }
    ...
    while (current && current.overordnet !== current.organisasjonsId) {
      ...
      const managerAzure = azureAdPersons.value.find(user => (user.onPremisesExtensionAttributes.extensionAttribute9 && current.leder) && ((user.onPremisesExtensionAttributes.extensionAttribute9 === current.leder.ansattnummer) || user.onPremisesExtensionAttributes.extensionAttribute9 === current.leder))
      unit.struktur.push( { organisasjonsId: current.organisasjonsId, navn: current.navn, leder: managerAzure ? managerAzure.userPrincipalName : 'Ukjent leder', ledernavn: managerAzure ? managerAzure.displayName : 'Ukjent ledernavn' } )
    }
  ...
  const bakedPersons = employeePersons.map(person => {
    ...
    const baked = {
      ...
      arbeidsforhold: employeePositions.filter(position => (position.systemId !== unknownValue && employeeResource.arbeidsforhold.includes(position.systemId))).map(forhold => { // Find the employeePositions related to the employeeResource - map the result
        ...
        // add leaders mail address to leder
        if (withOrgInfo.arbeidssted?.leder && Array.isArray(withOrgInfo.arbeidssted?.struktur) && withOrgInfo.arbeidssted.struktur.length > 0) {
          const currentLeader = withOrgInfo.arbeidssted.struktur.find(p => p.ledernavn === withOrgInfo.arbeidssted.leder.navn)
          if (currentLeader) {
            withOrgInfo.arbeidssted.leder.upn = currentLeader.leder
          }
        }
